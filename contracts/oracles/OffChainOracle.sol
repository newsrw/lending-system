...
contract OffChainOracle is IOracle, Initializable, AccessControlUpgradeable {
    ...
    modifier onlyFeeder() {
        require(
            hasRole(FEEDER_ROLE, _msgSender()),
            "OffChainOracle::onlyFeeder::only FEEDER role"
        );
        _;
    }
    ...
    function setPrices(
        ...
        require(
            _token1s.length == _len,
            "OffChainOracle::setPrices:: bad token1s length"
        );
        require(
            _prices.length == _len,
            "OffChainOracle::setPrices:: bad prices length"
        );
        for (uint256 idx = 0; idx < _len; idx++) {
            address _token0 = _token0s[idx];
            address _token1 = _token1s[idx];
            uint256 _price = _prices[idx];
            store[_token0][_token1] = PriceData({
    ...
    function _getPrice(address _token0, address _token1)
        internal
        view
        returns (uint256 price, uint256 lastUpdate)
    {
        ...
        require(
            price != 0 && lastUpdate != 0,
            "OffChainOracle::getPrice:: bad price data"
        );
